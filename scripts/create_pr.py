...
Creates a pull request for the current topic branch to 'main' using the GitHub REST API
and credentials resolved from GITHUB_TOKEN or the system git credential helper (osxkeychain).
"""
...
def get_github_token() -> str:
    ...
    # 2. Fall back to git credential helper
    try:
...
def get_repo_owner_and_name() -> tuple[str, str]:
    ...
    # Handle ssh or https formats: git@github.com:owner/repo.git or https://github.com/owner/repo.git
...
def create_pull_request(title: str, body: str, base: str = "main", draft: bool = False) -> dict:
...
def main():
    ...
    try:
        pr = create_pull_request(args.title, args.body, args.base, args.draft)
        print(f"✅ Pull Request successfully created: {pr.get('html_url')}")
